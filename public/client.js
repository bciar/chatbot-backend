    ...
    var Botkit = {
      config: {
        ws_url: (location.protocol === 'https:' ? 'wss' : 'ws') + '://' + location.host + '/chatsocket',
      ...
      request: function(url, body) {
        // If it's production, append /chatbot before all urls
      ...
      getRecruiter: function() {
        ...
        if (that.current_user) {
          that.request('/recruiter', {
            org: that.current_user
          }).then(function(recruiter) {
            if (recruiter.success) {
              that.recruiter = recruiter.recruiter;
              that.trigger('recruiter_loaded', recruiter.recruiter);
            } else {
              that.trigger('recruiter_error', new Error(recruiter.error));
            }
          }).catch(function(err) {
            console.warn("Error received", err)
            that.trigger('recruiter_error', err);
          });
        } else {
          console.warn("No current user, can't fetch recruiter")
        }
      },
      ...
      connect: function(user) {
        ...
        if (user) {
          if (user.id) {
            Botkit.setCookie('hirehumanly_guid', user.id, 1);
          }
      ...
      connectWebhook: function() {
        var that = this;
        if (Botkit.getCookie('hirehumanly_guid')) {
          that.guid = Botkit.getCookie('hirehumanly_guid');
          connectEvent = 'welcome_back';
        } else {
          that.guid = that.generate_guid();
          Botkit.setCookie('hirehumanly_guid', that.guid, 1);
        }

        if (! ('candidate_guid' in that.current_user)) {
          that.current_user.candidate_guid = that.guid;
        }
      ...
      boot: function(user) {
        ...
        that.on('recruiter_loaded', function(recruiter) {
          if (recruiter) {
            var source = document.getElementById('recruiter-template').innerHTML;
            that.recruiter_template = Handlebars.compile(source);

            var recruiterHeader = document.getElementById('chat__header__container');
            recruiterHeader.innerHTML = that.recruiter_template(recruiter);
